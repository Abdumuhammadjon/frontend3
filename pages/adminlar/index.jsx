...
export default function Dashboard() {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      const response = await axios.post("https://backed1.onrender.com/auth/register", userData);
  ...
  return (
    <div className="flex flex-col -ml-5  min-h-screen bg-gray-100">
  {/* HEADER */}
  ...
  <div className="flex flex-1 pt-16">
    {/* SIDEBAR */}
    <div className={`bg-gray-900 text-white fixed md:relative top-16 md:top-0 h-full md:h-auto p-5 transition-all duration-300 z-20 ${isOpen ? "w-64" : "w-20"} flex flex-col`}>
      <button
        className="text-white mb-6 focus:outline-none self-end md:hidden"
        onClick={() => setIsOpen(!isOpen)}
      >
        <Menu size={24} />
      </button>
      {isOpen && <h2 className="text-2xl font-bold mb-6 hidden md:block">Dashboard</h2>}
    ...
    {/* FORM CONTENT */}
    <div className={`flex-1 p-4 md:p-8 pt-20 transition-all duration-300 w-full ${isOpen ? "md:ml-64" : "md:ml-20"}`}>
      <div className="mt-10 p-6 w-full max-w-2xl border rounded bg-white">
